	    			...
	    			<table data-role="table" id="movie-table-custom" data-mode="reflow" class="movie-list ui-responsive">
						  <thead>
						    <tr>
						      ...
						      <th data-priority="2">积分</th>
						      <th data-priority="3" style="width:45%">积分来源</th>
						      <th data-priority="4">mac地址</th>
						      <th data-priority="5">时间</th>
						    </tr>
						  </thead>
						  <tbody>
						  	<c:forEach items="${creditsList}" var="addCredit">
						  		<tr>
							      <th>${addCredit.account}</th>
							      <th>${addCredit.credit}</th>
							      <c:if test="${addCredit.channelType eq '1'}">
							      		<td>下载软件</td>
							      </c:if>
							      <c:if test="${addCredit.channelType eq '2'}">
							      		<td>抽奖渠道</td>
							      </c:if>
							      <c:if test="${addCredit.channelType eq '3'}">
							      		<td>邀请用户</td>
							      </c:if>
							      <c:if test="${addCredit.channelType eq '4'}">
							      		<td>赌博游戏</td>
							      </c:if>
							      <c:if test="${addCredit.channelType eq '5'}">
							      		<td>彩票奖励</td>
							      </c:if>
							      <c:if test="${addCredit.channelType eq '6'}">
							      		<td>分享渠道</td>
							      </c:if>
							      <td>${addCredit.macAddress}</td>
							      <td>${addCredit.createTime}</td>
							    </tr>
						  	</c:forEach>
						  </tbody>
						</table>
